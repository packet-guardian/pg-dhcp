...
type Handler struct {
	gatewayCache map[string]*sys.Network
...
func NewDHCPServer(conf *sys.Config, s *ServerConfig) *Handler {
	if s.Log == nil {
		s.Log = createLogger()
	}
	c = conf
	...
	return &Handler{
		c:            s,
		gatewayCache: make(map[string]*sys.Network),
		gatewayMutex: sync.Mutex{},
	}
}
