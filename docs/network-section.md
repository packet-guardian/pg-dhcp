...
- `ignore-registration` - The registration status of a device will not matter in determining which pool to use. When configuring the network, configure an unregsitered subnet.
- `enforce-blocklist` - Normally a blocked device would be treated as unregistered. This options ensures blocked device do not receive a leases from the configured network.
